...
macro(append_global_list property_name)
...
macro(register_component)
    set_property(GLOBAL PROPERTY COMPONENT_META_${COMPONENT_NAME}_MODULE_NAME "${MODULE_NAME}")
    set_property(GLOBAL PROPERTY COMPONENT_META_${COMPONENT_NAME}_AUTO_DEF "${AUTO_DEF_FILE_ID}")
    set_property(GLOBAL PROPERTY COMPONENT_META_${COMPONENT_NAME}_SOURCES "${SOURCES}")

    # backward compatibility for existing macros/scripts that still read variables
    set(${COMPONENT_NAME}_MODULE_NAME ${MODULE_NAME})
    set(${COMPONENT_NAME}_AUTO_DEF ${AUTO_DEF_FILE_ID})
    set(${COMPONENT_NAME}_SOURCES ${SOURCES})
...
macro(main_component)
    if(DEFINED MAIN_COMPONENT AND MAIN_COMPONENT STREQUAL true)
        set_property(GLOBAL PROPERTY SDK_PROJECT_FILE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")
        set_property(GLOBAL PROPERTY MAIN_COMPONENT "${COMPONENT_NAME}")

        # backward compatibility
        set(SDK_PROJECT_FILE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")
        set(MAIN_COMPONENT "${COMPONENT_NAME}")
    endif()
endmacro()

macro(add_to_logdef)
    if(DEFINED LOG_DEF)
        set_property(GLOBAL APPEND PROPERTY LOG_DEF_LIST "${LOG_DEF}")
        set(LOG_DEF_LIST "${LOG_DEF_LIST}" "${LOG_DEF}")
    endif()
...
macro(build_component)
    ...
    elseif(DEFINED COMPONENT_ON_ROM)
        ...
        if(NOT ${PUBLIC_HEADER_NUM} EQUAL 0)
            append_global_list(ALL_PUBLIC_HEADER ${PUBLIC_HEADER})
            set(ALL_PUBLIC_HEADER "${ALL_PUBLIC_HEADER}" "${PUBLIC_HEADER}")
        endif()

        if(DEFINED LOG_CUSTOM_ENABLE)
            if(${LOG_CUSTOM_ENABLE} STREQUAL True)
                if(NOT ${PUBLIC_HEADER_NUM} EQUAL 0)
                    append_global_list(ALL_HEADER_DIRS ${PUBLIC_HEADER})
                    set(ALL_HEADER_DIRS "${ALL_HEADER_DIRS}" "${PUBLIC_HEADER}")
                endif()

                list(LENGTH PRIVATE_HEADER PRIVATE_HEADER_NUM)
                if(NOT ${PRIVATE_HEADER_NUM} EQUAL 0)
                    append_global_list(ALL_HEADER_DIRS ${PRIVATE_HEADER})
                    set(ALL_HEADER_DIRS "${ALL_HEADER_DIRS}" "${PRIVATE_HEADER}")
                endif()

                list(LENGTH PUBLIC_DEFINES PUBLIC_DEFINES_NUM)
                if(NOT ${PUBLIC_DEFINES_NUM} EQUAL 0)
                    append_global_list(ALL_DEFINES ${PUBLIC_DEFINES})
                    set(ALL_DEFINES "${ALL_DEFINES}" "${PUBLIC_DEFINES}")
                endif()

                list(LENGTH PRIVATE_DEFINES PRIVATE_DEFINES_NUM)
                if(NOT ${PRIVATE_DEFINES_NUM} EQUAL 0)
                    append_global_list(ALL_DEFINES ${PRIVATE_DEFINES})
                    set(ALL_DEFINES "${ALL_DEFINES}" "${PRIVATE_DEFINES}")
                endif()

                list(LENGTH DEFINES DEFINES_NUM)
                if(NOT ${DEFINES_NUM} EQUAL 0)
                    append_global_list(ALL_DEFINES ${DEFINES})
                    set(ALL_DEFINES "${ALL_DEFINES}" "${DEFINES}")
                endif()
            endif()
        endif()

        append_global_list(ALL_PUBLIC_DEFINES ${PUBLIC_DEFINES})
        append_global_list(ALL_PUBLIC_CCFLAGS ${COMPONENT_PUBLIC_CCFLAGS})
        set(ALL_PUBLIC_DEFINES "${ALL_PUBLIC_DEFINES}" "${PUBLIC_DEFINES}")
        set(ALL_PUBLIC_CCFLAGS "${ALL_PUBLIC_CCFLAGS}" "${COMPONENT_PUBLIC_CCFLAGS}")
        link_libs()
