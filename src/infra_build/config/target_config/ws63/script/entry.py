...
def config_check(root_path, target_name):
    config_file_path = os.path.join(root_path, "infra_build", "config", "target_config", "ws63", "config.py")
...
def build_sdk():
    ...
    errcode = exec_shell(["python3", "infra_build/script/cmake_build.py", "pack_ws63_sdk"], None, True)
    if errcode != 0:
...
def guard_sample(target_name, env):
    ...
    for cfg_path in env.get('menuconfigs', []):
        ...
        errcode = exec_shell(["python3", "infra_build/script/cmake_build.py", "-c", build_target], None, True)
        if errcode != 0:
