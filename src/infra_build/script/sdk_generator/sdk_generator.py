...
class SdkGenerator:
    ...
    def copy_menuconfig(self):
        for chip in self.chip:
            if os.path.exists(os.path.join(root_path, 'infra_build', 'config', 'target_config', chip, 'menuconfig')):
                self.copy_srcs([os.path.join(root_path, 'infra_build', 'config', 'target_config', chip, 'menuconfig')])
    ...
    def genarate_efuse_cfg_bin(self, chip_name):
        if self.env.get('generate_efuse_bin') == True:
    ...
    def genarate_sdk_target_config(self, targets):
        ...
        print(reserve)
        for chip in reserve:
            path = os.path.join(self.sdk_root_path, 'infra_build', 'config', 'target_config', chip)
    ...
    def sdk_delete_tmp_files(self):
        ...
        for dir_path, dir_names, file_names in os.walk(os.path.join(output_root, 'sdk', 'infra_build', 'config', 'target_config', self.env.get('chip')), topdown=False):
            for name in file_names:
    ...
    def sdk_build(self, build_time, nhso, build_level):
        ...
        for idx, target in enumerate(build_targets):
            sdk_build_cmd = [sys.executable, 'infra_build/script/cmake_build.py', target]
            if build_time != '':
    ...
    def close_sdk_mem_limit(self):
        if "SDK_NOT_MEM_LIMIT" not in self.env.config.get("defines", []):
            return
        chip_config_path = os.path.join('infra_build', 'config', 'target_config', self.env.get('chip'), 'target_config.py')
