...
/** SLE address array index constants */
#define SLE_ADDR_INDEX_0 0
#define SLE_ADDR_INDEX_4 4
#define SLE_ADDR_INDEX_5 5
...
static void sle_uart_client_sample_connect_state_changed_cbk(uint16_t conn_id,
...
static void sle_uart_client_sample_find_structure_cbk(uint8_t client_id,
                                                      uint16_t conn_id,
...
static void sle_uart_client_sample_find_property_cbk(uint8_t client_id,
                                                     uint16_t conn_id,
                                                     ssapc_find_property_result_t *property,
                                                     errcode_t status)
{
    osal_printk(
        "%s sle_uart_client_sample_find_property_cbk, client id: %d, conn id: %d, operate ind: %d, "
        "descriptors count: %d status:%d property->handle %d\r\n",
        SLE_UART_CLIENT_LOG, client_id, conn_id, property->operate_indication, property->descriptors_count, status,
        property->handle);
    g_sle_uart_send_param.handle = property->handle;
    g_sle_uart_send_param.type = SSAP_PROPERTY_TYPE_VALUE;
}

static void sle_uart_client_sample_find_structure_cmp_cbk(uint8_t client_id,
                                                          uint16_t conn_id,
...
static void sle_uart_client_sample_write_cfm_cb(uint8_t client_id,
                                                uint16_t conn_id,
