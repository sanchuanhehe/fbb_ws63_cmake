/**！
 * @file beetle_mqtt_sample.c
 * @brief The development board connects to an MQTT server to publish and subscribe to messages.
...
char *g_msg = CONFIG_MQTT_PUB_MSG;
MQTTClient client;
...
static errcode_t mqtt_connect(void)
{
    ...
    MQTTClient_init();
    /* 创建 MQTT 客户端 */
    ret = MQTTClient_create(&client, CONFIG_SERVER_IP_ADDR, CLIENT_ID, MQTTCLIENT_PERSISTENCE_NONE, NULL);
    if (ret != MQTTCLIENT_SUCCESS) {
...
#ifdef CONFIG_MQTT_AUTH_ENABLE
    // 设置MQTT服务器认证信息
    conn_opts.username = CONFIG_MQTT_USERNAME;
    conn_opts.password = CONFIG_MQTT_PASSWORD;
#endif
    ...
    // 订阅MQTT主题
    mqtt_subscribe(CONFIG_MQTT_TOPIC_SUB);
    while (1) {
        osDelay(100); /* 100: 延时1s  */
        mqtt_publish(CONFIG_MQTT_TOPIC_PUB, g_msg);
    }
